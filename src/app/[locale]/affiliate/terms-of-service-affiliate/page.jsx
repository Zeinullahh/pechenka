...
const TERMS_BUNDLES = {
  en: enMessages.affiliate.termsAff,
  ru: ruMessages.affiliate.termsAff,
};

const resolveKey = (bundle, key) => {
  const path = key.replace("affiliate.termsAff.", "").split(".");
  let value = bundle;
...
const AffiliateTermsOfServicePage = () => {
  ...
  const [termsLangOverride, setTermsLangOverride] = useState(null);

  const termsLang = termsLangOverride ?? (language === "ru" ? "ru" : "en");

  const tt = (key, fallback) =>
    resolveKey(TERMS_BUNDLES[termsLang], key) ?? fallback ?? key;
      ...
      <main className="flex min-h-screen flex-col bg-black text-white">
        ...
        <div className="container mx-auto flex w-full max-w-7xl flex-1 flex-col gap-8 px-4 pb-24 pt-32 sm:px-6 lg:flex-row lg:px-8">
          ...
          <article className="flex-1">
            <div
              className="mb-8 inline-flex gap-1 rounded-lg border border-white/15 bg-white/5 p-1"
              role="group"
            ...
            >
              <button
                ...
                onClick={() => setTermsLangOverride("en")}
              >
                English
              </button>
              <button
                type="button"
                className={langButtonClass("ru")}
                onClick={() => setTermsLangOverride("ru")}
              >
                Русский
              </button>
            </div>
